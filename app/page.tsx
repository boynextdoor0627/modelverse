...
import { estimateMemory, estimateRoofline } from "./deployment-math";
...
type Model = {
  ...
  verificationStatus?: "official" | "preview" | "third-party" | "unverified"; variantOf?: string;
};
type Catalog = { meta: { modelCount: number; companyCount: number; generatedAt: string; sourceNote: string }; models: Model[] };
type OfficialRegistry = { updatedAt: string; models: Array<{ company: string; name: string; source: string; lifecycle: string }> };
...
type IconName = "close" | "plus" | "minus" | "reset" | "calculator" | "motion" | "models" | "agents" | "filter" | "compare";
function UiIcon({ name }: { name: IconName }) {
  const paths: Record<IconName, React.ReactNode> = {
    close: <><path d="M6 6l12 12"/><path d="M18 6 6 18"/></>,
    plus: <><path d="M12 5v14"/><path d="M5 12h14"/></>, minus: <path d="M5 12h14"/>,
    reset: <><path d="M4.8 9A8 8 0 1 1 4 14"/><path d="M4.8 4.8V9H9"/></>,
    calculator: <><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M8 7h8M8 11h2m4 0h2m-8 4h2m4 0h2m-8 4h2m4 0h2"/></>,
    motion: <><circle cx="12" cy="12" r="7"/><path d="M12 5a7 7 0 0 1 0 14"/></>,
    models: <><circle cx="12" cy="12" r="3"/><path d="M4.5 8.5c2-3.5 13-3.5 15 0s-6 10.5-9.5 9-7.5-6-5.5-9Z"/></>,
    agents: <><circle cx="12" cy="8" r="3"/><path d="M6.5 20c.7-4 2.5-6 5.5-6s4.8 2 5.5 6"/><path d="M4 10h2m12 0h2"/></>,
...
function sourceTrust(model: Model, lang: Lang) {
  const status = model.verificationStatus || (model.sourceKind === "huggingface" ? "official" : model.sourceKind === "openrouter" ? "third-party" : "unverified");
  const labels = lang === "zh"
    ? { official:"官方发布 · 已核验", preview:"官方预览 · 可能调整", "third-party":"第三方平台收录", unverified:"人工整理 · 待官方复核" }
    : { official:"Official release · Verified", preview:"Official preview · Subject to change", "third-party":"Third-party catalog listing", unverified:"Curated · Pending official review" };
  return `${labels[status]} · ${popularityText(model, lang)}`;
}
...
const GPU_PROFILES = [
  { id:"24", memory:22, bandwidth:1008, bf16:165, interconnect:64, price:16000, zh:"RTX 4090 24 GB · 可用约 22 GB", en:"RTX 4090 24 GB · ~22 GB usable" },
  { id:"48", memory:44, bandwidth:864, bf16:362, interconnect:64, price:40000, zh:"L20 / L40S 48 GB · 可用约 44 GB", en:"L20 / L40S 48 GB · ~44 GB usable" },
  { id:"84", memory:77, bandwidth:1792, bf16:500, interconnect:64, price:70000, zh:"RTX PRO 6000D 84 GB · 可用约 77 GB", en:"RTX PRO 6000D 84 GB · ~77 GB usable" },
  { id:"96", memory:88, bandwidth:4000, bf16:148, interconnect:900, price:130000, zh:"H20 96 GB · 可用约 88 GB", en:"H20 96 GB · ~88 GB usable" },
  { id:"141", memory:130, bandwidth:4800, bf16:1979, interconnect:900, price:400000, zh:"H200 SXM 141 GB · 可用约 130 GB", en:"H200 SXM 141 GB · ~130 GB usable" },
];
const FRAMEWORKS = [
  { id:"paged", factor:1.1, decodeEfficiency:.72, prefillEfficiency:.58, zh:"vLLM + PagedAttention · 10%", en:"vLLM + PagedAttention · 10%" },
  { id:"vllm", factor:1.15, decodeEfficiency:.66, prefillEfficiency:.54, zh:"vLLM 标准 · 15%", en:"Standard vLLM · 15%" },
  { id:"tgi", factor:1.2, decodeEfficiency:.58, prefillEfficiency:.48, zh:"TGI · 20%", en:"TGI · 20%" },
  { id:"hf", factor:1.25, decodeEfficiency:.48, prefillEfficiency:.4, zh:"HF Transformers · 25%", en:"HF Transformers · 25%" },
];
const CLOUD_APIS = [
...
type ModelArchitecture = { match: RegExp; layers: number; kvHeads: number; headDim: number; label: string };
const MODEL_ARCHITECTURES: ModelArchitecture[] = [
  { match:/Llama 3\.1 8B/i, layers:32, kvHeads:8, headDim:128, label:"Llama 3.1 8B GQA" },
  { match:/Llama 3\.(?:1|3) 70B/i, layers:80, kvHeads:8, headDim:128, label:"Llama 70B GQA" },
  { match:/Qwen3[- ]235B/i, layers:94, kvHeads:4, headDim:128, label:"Qwen3 235B GQA" },
  { match:/Qwen3[- ]30B/i, layers:48, kvHeads:4, headDim:128, label:"Qwen3 30B GQA" },
  { match:/gpt[- ]oss[- ]120b/i, layers:36, kvHeads:8, headDim:64, label:"GPT-OSS 120B GQA" },
  { match:/gpt[- ]oss[- ]20b/i, layers:24, kvHeads:8, headDim:64, label:"GPT-OSS 20B GQA" },
  { match:/Mistral Small 3\.2 24B/i, layers:40, kvHeads:8, headDim:128, label:"Mistral Small GQA" },
];
function architectureFor(model: Model) { const text = `${model.name} ${model.id}`; return MODEL_ARCHITECTURES.find((item) => item.match.test(text)); }
...
const DEPLOYMENT_FEATURED_MODELS = [
  "Qwen3.8-27B", "DeepSeek-V4-Pro", "DeepSeek-V4-Flash", "qwen3-235B-A22B",
  "qwen3-Coder", "qwen3-30B-A3B", "Qwen3-Next-80B-A3B-Thinking", "kimi-k3",
  "glm-5.2", "MiniMax-M3", "gpt-oss-120b", "gpt-oss-20b", "Llama 3.3 70B Instruct",
  "Llama 3.1 8B Instruct", "Nemotron 3 Nano 30B A3B", "Mistral: Mistral Small 3.2 24B",
  "Seed-OSS-36B-Instruct",
];
function kvCoefficient(model: Model) {
  const architecture = architectureFor(model);
  if (architecture) return 2 * architecture.layers * architecture.kvHeads * architecture.headDim * 2 / 1024;
...
function DeploymentLab({ model, models, lang, updatedAt, onModelChange, onClose }: { model: Model; models: Model[]; lang: Lang; updatedAt: string; onModelChange: (model: Model) => void; onClose: () => void }) {
  ...
  const memoryEstimate = estimateMemory({ totalParamsB:paramsB, bytesPerParameter:PRECISIONS[precision].bytes, kvKBPerToken:kvKB, contextK, concurrency, frameworkFactor:framework.factor });
  const weights = memoryEstimate.weightsGB;
  const kv = memoryEstimate.kvGB;
  const recommended = memoryEstimate.totalGB;
  ...
  const activeParamsB = isMoe && parseMetric(model.activeParams) > 0 ? parseMetric(model.activeParams) : paramsB;
  const communicationEfficiency = recommendedCards <= 1 ? 1 : recommendedCards <= 8 ? (gpu.interconnect >= 600 ? .9 : .78) : .68;
  const roofline = estimateRoofline({ activeParamsB, bytesPerParameter:PRECISIONS[precision].bytes, promptTokens, generationTokens, gpuBandwidthGBs:gpu.bandwidth, gpuBf16Tflops:gpu.bf16, gpuCount:recommendedCards, decodeEfficiency:framework.decodeEfficiency, prefillEfficiency:framework.prefillEfficiency, communicationEfficiency });
  const { decodeTokensPerSecond, prefillMs, requestLatencySeconds } = roofline;
  const architectureMethod = architecture
    ? (lang === "zh" ? `${architecture.label} · 精确结构公式` : `${architecture.label} · architecture formula`)
    : (KV_OVERRIDES[model.id] || /MLA|DeepSeek/i.test(`${model.name} ${model.type} ${model.notes}`))
      ? (lang === "zh" ? "模型校准 KV 系数" : "Model-calibrated KV coefficient")
      : (lang === "zh" ? "保守 KV 经验值" : "Conservative KV heuristic");
  ...
  const labels = lang === "zh" ? {
    kicker:"DEPLOYMENT LAB · ENGINEERING", title:"AI 部署选型计算器", intro:"把模型、业务负载、硬件容量与性能放在同一个决策界面。", choose:"选择模型", precision:"权重精度", context:"实际上下文", concurrency:"峰值并发", prompt:"平均输入 Token", generation:"平均生成 Token", kv:"KV Cache 精度", framework:"推理框架与开销", gpu:"GPU 与可用显存", weights:"模型权重", cache:"KV Cache", runtime:"框架开销", total:"总显存需求", minimum:"数学最低", suggested:"工程建议", cards:"张", result:"容量结论", performance:"性能估算", decode:"单请求 Decode", ttft:"首 Token 延迟", latency:"请求总时延", bandwidth:"总显存带宽", assumptions:"计算假设", back:"返回模型宇宙", warning:"当前目录未确认该模型可下载权重；以下仅做容量演算，不代表可以本地部署。", moe:"MoE 模型：显存按总参数估算，性能按激活参数近似，避免低估权重容量。", dense:"Dense 模型跨卡越多，通信开销通常越明显。", kvTip:"KV Cache 已超过模型权重，建议缩短上下文、降低并发或使用 INT8 KV。", estimate:"性能为带宽／算力 Roofline 估算，不代替推理框架实测、硬件兼容性验证和 POC。", formula:"总需求 =（总参数权重 + KV Cache × 并发）× 框架开销；MoE 不用激活参数代替驻留权重。", updated:"模型数据更新", source:"模型参数来源"
  } : {
    kicker:"DEPLOYMENT LAB · ENGINEERING", title:"AI deployment sizing calculator", intro:"Bring model, workload, hardware capacity, and performance into one decision surface.", choose:"Choose model", precision:"Weight precision", context:"Working context", concurrency:"Peak concurrency", prompt:"Average input tokens", generation:"Average output tokens", kv:"KV cache precision", framework:"Inference framework overhead", gpu:"GPU and usable memory", weights:"Model weights", cache:"KV cache", runtime:"Framework overhead", total:"Total VRAM required", minimum:"Mathematical minimum", suggested:"Engineering plan", cards:"cards", result:"Capacity result", performance:"Performance estimate", decode:"Single-request decode", ttft:"Time to first token", latency:"Request latency", bandwidth:"Aggregate bandwidth", assumptions:"Assumptions", back:"Back to model universe", warning:"Downloadable weights are not confirmed in the catalog. This is capacity modeling only, not proof of deployability.", moe:"MoE memory follows total parameters; performance is approximated from active parameters so resident weights are not understated.", dense:"Dense models generally incur more communication overhead as card count grows.", kvTip:"KV cache exceeds model weights. Consider shorter context, lower concurrency, or INT8 KV.", estimate:"Performance is a bandwidth/compute roofline estimate, not a substitute for framework benchmarks, hardware validation, or a POC.", formula:"Total = (total resident weights + KV cache × concurrency) × framework overhead; MoE active parameters never replace resident weights.", updated:"Model data updated", source:"Model parameter source"
  };
  ...
  return <section className="deployment-lab" aria-label={`${model.name} ${labels.title}`}>
    <button className="deployment-close" onClick={onClose} aria-label={labels.back} title={labels.back}><UiIcon name="close" /></button>
    ...
    <div className="deployment-layout"><form className="deployment-controls" onSubmit={(event) => event.preventDefault()}>
      ...
      <div className="deployment-pair"><label><span>{labels.prompt}</span><input type="number" min="1" max="1000000" step="128" value={promptTokens} onChange={(event) => setPromptTokens(Math.max(1, Number(event.target.value) || 1))} /></label><label><span>{labels.generation}</span><input type="number" min="1" max="1000000" step="128" value={generationTokens} onChange={(event) => setGenerationTokens(Math.max(1, Number(event.target.value) || 1))} /></label></div>
      ...
      <label><span>{labels.gpu}</span><select value={gpuId} onChange={(event) => setGpuId(event.target.value)}>{GPU_PROFILES.map((item) => <option key={item.id} value={item.id}>{lang === "zh" ? item.zh : item.en}</option>)}</select><small>{gpu.bandwidth.toLocaleString()} GB/s · {gpu.bf16.toLocaleString()} BF16 TFLOPS · {lang === "zh" ? "公开规格与工程参照" : "public specification and engineering reference"}</small></label>
      ...
      <section className="deployment-performance"><small>{labels.performance} · ROOFLINE</small><div><article><span>{labels.decode}</span><b>{format(decodeTokensPerSecond)} tok/s</b></article><article><span>{labels.ttft}</span><b>{format(prefillMs)} ms</b></article><article><span>{labels.latency}</span><b>{format(requestLatencySeconds)} s</b></article><article><span>{labels.bandwidth}</span><b>{format(gpu.bandwidth * recommendedCards)} GB/s</b></article></div><p>{architectureMethod} · {lang === "zh" ? `通信效率 ${Math.round(communicationEfficiency * 100)}%` : `${Math.round(communicationEfficiency * 100)}% communication efficiency`}</p></section>
    </div></div>
...
function ModelDetailSheet({ model, lang, updatedAt, tier, compared, onCompare, onDeploy, onShare, onClose, copied }: { model: Model; lang: Lang; updatedAt: string; tier: ModelTier; compared: boolean; onCompare: () => void; onDeploy: () => void; onShare: () => void; onClose: () => void; copied: boolean }) {
  ...
  return <aside className="model-sheet" aria-label={`${model.name} ${labels.specs}`}>
    <button className="sheet-close" onClick={onClose} aria-label={lang === "zh" ? "关闭详情" : "Close details"} title={lang === "zh" ? "关闭详情" : "Close details"}><UiIcon name="close" /></button>
    ...
    <footer className="sheet-footer"><button className="deploy-cta" onClick={onDeploy}><UiIcon name="calculator" />{labels.deploy}</button><button className={compared ? "primary active" : "primary"} onClick={onCompare}>{compared ? `✓ ${labels.compared}` : <><UiIcon name="plus" />{labels.compare}</>}</button><button onClick={onShare}>{copied ? labels.copied : labels.share}</button>{model.source.startsWith("http") ? <a href={model.source} target="_blank" rel="noreferrer">{labels.source} ↗</a> : <span>{labels.unavailable}</span>}</footer>
  </aside>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("/models.json").then((response) => response.json() as Promise<Catalog>),
      fetch("/official-model-registry.json").then((response) => response.json() as Promise<OfficialRegistry>),
    ]).then(([data, officialRegistry]) => {
      const evidence = new Map(officialRegistry.models.map((model) => [`${canonicalCompany(model.company).toLowerCase()}::${model.name.toLowerCase()}`, model]));
      const models = data.models.map((model) => {
        const company = canonicalCompany(model.company);
        const verified = evidence.get(`${company.toLowerCase()}::${model.name.toLowerCase()}`);
        return verified ? { ...model, company, source:verified.source, lifecycle:verified.lifecycle, verificationStatus:verified.lifecycle === "preview" ? "preview" as const : "official" as const, discoveredAt:officialRegistry.updatedAt } : { ...model, company };
      }).filter((model) => model.sourceKind !== "curated" || model.verificationStatus === "official" || model.verificationStatus === "preview");
      setCatalog({ ...data, meta:{ ...data.meta, modelCount:models.length, companyCount:new Set(models.map((model) => model.company)).size, generatedAt:officialRegistry.updatedAt }, models });
    });
  }, []);
  ...
  const deploymentModels = DEPLOYMENT_FEATURED_MODELS.map((name) => catalog.models.find((model) => model.name.toLowerCase() === name.toLowerCase())).filter((model): model is Model => Boolean(model));
  ...
  return <main className={`universe view-${viewMode} motion-${motionMode} ${focusedCompany ? "is-focused" : "is-overview"} ${selected ? "has-selection" : ""} ${compareOpen ? "has-compare" : ""} ${mobileFiltersOpen ? "has-mobile-filters" : ""} ${deploymentModel ? "has-deployment" : ""}`} lang={lang}>
    <header className="topbar">
      ...
      <div className="top-actions"><div className="view-switch"><button className={viewMode === "models" ? "active" : ""} onClick={() => setViewMode("models")}>{lang === "zh" ? "模型" : "Models"}</button><button className={viewMode === "agents" ? "active" : ""} onClick={() => { setViewMode("agents"); setSelected(null); setCompareOpen(false); }}>{lang === "zh" ? "Agent" : "Agents"}</button></div><button className="deployment-trigger" onClick={() => { setCompareOpen(false); setMobileFiltersOpen(false); setDeploymentModel(selected || deploymentDefault); }}><UiIcon name="calculator" />{lang === "zh" ? "部署计算器" : "Deploy calculator"}</button><button className="motion-toggle" onClick={cycleMotion} aria-label={lang === "zh" ? "切换动态效果" : "Change motion"} title={lang === "zh" ? "切换动态效果" : "Change motion"}><UiIcon name="motion" /></button><button className={`compare-trigger ${comparison.length ? "ready" : ""}`} onClick={() => setCompareOpen(true)}>{t.compare}<b>{comparison.length}</b></button><div className="language"><button className={lang === "zh" ? "active" : ""} onClick={() => setLang("zh")}>中文</button><button className={lang === "en" ? "active" : ""} onClick={() => setLang("en")}>EN</button></div></div>
    </header>
    ...
    <section className="space"><canvas ref={canvasRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd} onPointerLeave={onPointerLeave} onWheel={onWheel} aria-label={t.canvasLabel} />
      ...
      <div className="view-controls"><button onClick={() => zoomBy(.82)} aria-label={lang === "zh" ? "放大" : "Zoom in"} title={lang === "zh" ? "放大" : "Zoom in"}><UiIcon name="plus" /></button><span>{zoom}%</span><button onClick={() => zoomBy(1.22)} aria-label={lang === "zh" ? "缩小" : "Zoom out"} title={lang === "zh" ? "缩小" : "Zoom out"}><UiIcon name="minus" /></button><button onClick={resetView} aria-label={t.reset} title={t.reset}><UiIcon name="reset" /></button></div>
    ...
    <small className="site-credit">{lang === "zh" ? "上海引众 AI 事业部团队制作" : "CREATED BY SHANGHAI YINZHONG AI TEAM"}</small>
    <nav className="mobile-dock" aria-label={lang === "zh" ? "主要导航" : "Primary navigation"}><button className={viewMode === "models" ? "active" : ""} onClick={() => { setViewMode("models"); setMobileFiltersOpen(false); }}><i><UiIcon name="models" /></i><span>{lang === "zh" ? "模型" : "Models"}</span></button><button className={viewMode === "agents" ? "active" : ""} onClick={() => { setViewMode("agents"); setSelected(null); setCompareOpen(false); setMobileFiltersOpen(false); }}><i><UiIcon name="agents" /></i><span>Agent</span></button><button onClick={() => { setCompareOpen(false); setMobileFiltersOpen(false); setDeploymentModel(selected || deploymentDefault); }}><i><UiIcon name="calculator" /></i><span>{lang === "zh" ? "计算" : "Deploy"}</span></button><button className={mobileFiltersOpen ? "active" : ""} onClick={() => setMobileFiltersOpen(true)}><i><UiIcon name="filter" /></i><span>{lang === "zh" ? "筛选" : "Filter"}</span></button><button className={comparison.length ? "active" : ""} onClick={() => { setCompareOpen(true); setMobileFiltersOpen(false); }}><i><UiIcon name="compare" />{comparison.length > 0 && <b>{comparison.length}</b>}</i><span>{lang === "zh" ? "对比" : "Compare"}</span></button></nav>
