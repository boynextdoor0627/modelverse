...
function ModelDetailSheet({ model, lang, updatedAt, tier, compared, onCompare, onShare, onClose, copied }: { model: Model; lang: Lang; updatedAt: string; tier: ModelTier; compared: boolean; onCompare: () => void; onShare: () => void; onClose: () => void; copied: boolean }) {
...
export default function Home() {
  ...
  return <main className={`universe view-${viewMode} motion-${motionMode} ${focusedCompany ? "is-focused" : "is-overview"} ${selected ? "has-selection" : ""} ${compareOpen ? "has-compare" : ""} ${mobileFiltersOpen ? "has-mobile-filters" : ""}`} lang={lang}>
    ...
    {selected && selectedTier && <ModelDetailSheet model={selected} lang={lang} updatedAt={catalog.meta.generatedAt} tier={selectedTier} compared={comparison.some((item) => item.id === selected.id)} onCompare={() => toggleCompare(selected)} onShare={shareCurrent} onClose={() => setSelected(null)} copied={Boolean(shareNotice)} />}
  </main>;
}
